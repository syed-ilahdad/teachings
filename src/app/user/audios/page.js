...
export default function page() {
  ...
  useEffect(() => {
    Promise.all([
      fetch('/api/files?type=audio&limit=1000').then(r=>r.json()),
  ...
  const display = files
    .filter(f => {
      const s = !search || f.original_name.toLowerCase().includes(search.toLowerCase()) || (f.tags||'').toLowerCase().includes(search.toLowerCase()) || (f.title||'').toLowerCase().includes(search.toLowerCase());
  ...
  return (
    <UserLayout>
      <PageHeader title="🎵 Audio Files" subtitle={`${display.length} audio file${display.length!==1?'s':''}`} />
      ...
      {loading ? <div style={{textAlign:'center',padding:60}}><div className="spin" style={{width:28,height:28,border:'3px solid var(--border)',borderTopColor:'var(--accent)',borderRadius:'50%',margin:'0 auto'}} /></div>
        : display.length===0 ? <EmptyState icon="🎵" message="No audio files yet." />
