...
export default function page() {
  ...
  useEffect(() => {
    Promise.all([
      fetch('/api/files?must_watch=true&limit=500').then(r=>r.json()),
  ...
  return (
    <UserLayout>
      <PageHeader title="🎬 Must Watch" subtitle="Hand-picked must-watch content" />
      ...
      {loading ? <div style={{textAlign:'center',padding:60}}><div className="spin" style={{width:28,height:28,border:'3px solid var(--border)',borderTopColor:'var(--accent)',borderRadius:'50%',margin:'0 auto'}} /></div>
        : display.length===0 ? <EmptyState icon="🎬" message="No must-watch content yet." />
